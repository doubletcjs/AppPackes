...
class BackgroundServiceManager: NSObject {
    ...
    func onDetached() -> Bool {
        if #available(iOS 10.0, *) {
            BackgroundService.sharedInstance.run(action: BackgroundServiceAction.DETACHED)
        } else {
            // Fallback on earlier versions
            return false
        }
    ...
    func reLaunch() -> Bool {
        if #available(iOS 10.0, *) {
            BackgroundService.sharedInstance.run(action: BackgroundServiceAction.LAUNCH)
        } else {
            // Fallback on earlier versions
            return false
        }
